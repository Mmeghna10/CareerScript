...
const EditResume = () => {
  ...
  const validateAndNext = (e) => {
    ...
    switch (currentPage) {
      ...
      case "contact-info":
        ...
        if (!phone.trim()) errors.push("Valid 10 digits required is required");
        break; }
      ...
      case "education-info":
        resumeData.education.forEach(
          ({ degree, institution, startDate, endDate }, index) => {
            ...
            if (!institution.trim()) errors.push(`Institution is required ${index + 1}`);
            if (!startDate || !endDate)
              errors.push(
      ...
      case "projects":
        resumeData.projects.forEach(
          ({ title, description }, index) => {
            if (!title.trim())
              errors.push(`Title is required ${index + 1}`);
            if (!description.trim())
      ...
      case "certifications":
        resumeData.certifications.forEach(
          ({ title, issuer }, index) => {
           const t  = (title  ?? "").trim();
        const is = (issuer ?? "").trim();
       if (!t)
          errors.push(`Title is required ${index + 1}`);
        if (!is)
          errors.push(`Issuer is required ${index + 1}`);
          }
        );
        break;
      ...
      case "additionalInfo":
        return (
          <AdditionalInfoForm
            ...
            updateInterest={(index, value) => updateArrayItem("interests", index, null, value)}
          />
        );
  ...
  const updateArrayItem = (section, index, key, value) => {
    setResumeData((prev) => {
      ...
      // For interests array (simple string array)
      if (key === null) {
        updatedArray[index] = value;
      } else {
        // For complex objects like languages, skills etc
        updatedArray[index] = {
          ...updatedArray[index],
          [key]: value,
        };
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const { thumbnaiLink, profilePreviewUrl } = uploadResponse.data;

      console.log("RESUME_DATA__", resumeData);

      //Call the second API to update other resume data
      await updateResumeDetails(thumbnaiLink, profilePreviewUrl);
  ...
  const updateResumeDetails = async (thumbnaiLink, profilePreviewUrl) => {
    try{
      setIsLoading(true);
  ...
  const handleDeleteResume = async () => {
    try{
      ...
      toast.success("Resume Deleted uccessfully");
      navigate('/dashboard');
    }catch(err){
      console.error("Error capturing image", err);
    }finally{
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto">
        <div className="flex items-center justify-between gap-5 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-lg border border-slate-600 py-3 px-4 mb-4">
          <TitleInput
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-lg border border-slate-600 overflow-hidden">
